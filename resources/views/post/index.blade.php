        ...
        @if (isset($archive))
            ...
            <ul class="list-unstyled">
                @foreach ($archive as $link)
                    <li><a href="{{ route('blog.archive.index', ['year' => $link->year, 'month' => $link->month]) }}">{{ $link->month_name }} {{ $link->year }}</a> <span class="text-muted">({{ $link->post_count }})</span></li>
                @endforeach
            </ul>
        @endif
...
@if(Auth::check())
    @section('bottom')
        @foreach($posts as $post)
            @include('post.delete', ['id' => "delete-post-{$post->id}"])
        @endforeach
    @stop
@endif
