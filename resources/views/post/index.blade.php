...
@section ('content')
    @if (isset($year) || isset($tag))
        <p><a href="{{ url('blog') }}">&laquo; Blog index</a></p>
    ...
    <div class="row">
        <div class="col-md-8">
            @foreach ($posts as $post)
                ...
                <p class="text-muted">
                    Posted by {{ $post->owner->name }} {{ $post->created_at->diffForHumans() }}
                ...
                @if (Auth::check())
                    <div class="pull-right">
                        <ul class="list-inline">
                            ...
                            <li><a class="text-danger" href="#delete-item-{{ $post->id }}" data-toggle="modal" data-target="#delete-item-{{ $post->id }}">Delete Post</a></li>
                        </ul>
                    </div>
                @endif
        ...
        <div class="col-md-4 text-right">
            @if (isset($archive))
                ...
                <ul class="list-unstyled">
                    @foreach ($archive as $link)
                        <li><a href="{{ route('blog.post.archive.index', ['year' => $link->year, 'month' => $link->month]) }}">{{ $link->month_name }} {{ $link->year }}</a> <span class="text-muted">({{ $link->post_count }})</span></li>
                    @endforeach
                </ul>
            @endif
            ...
            @if (isset($tags))
                <h3>Tags</h3>
                @foreach ($tags as $tag)
                    <a href="{{ route('blog.post.tag.index', ['tag' => $tag->slug]) }}" class="label label-primary">{{ $tag->name }}</a>
                @endforeach
            @endif
        </div>
    </div>
